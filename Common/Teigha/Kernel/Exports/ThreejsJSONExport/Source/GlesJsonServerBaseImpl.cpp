...
void OdGlesJsonServerBaseImpl::DropDbStub(const char* pTagName, OdDbStub* pDbStub)
{
  if (!m_useFakeDbStub || !pDbStub)
    OdGLES2JsonServerBaseImpl::DropDbStub(pTagName, pDbStub);
  else
    ident(pTagName, OdAnsiString().format(PRIX64, ((OdFakeDbStub*) pDbStub)->handle).c_str(), OdGLES2JsonServer::kType); // ident(pTagName, (const char*) pDbStub);
