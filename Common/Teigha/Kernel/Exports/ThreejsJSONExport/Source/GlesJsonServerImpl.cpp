...
///////////////////////////////////////////////////////////////////////////////
//
// GlesJsonServerImpl.cpp
//

#include "OdaCommon.h"
#include "GlesJsonServerImpl.h"

OdGlesJsonServerImpl::OdGlesJsonServerImpl(const OdDbBaseDatabase *pDb) // = NULL
  : OdGlesJsonServerBaseImpl(pDb)
  , m_indexNextFree(0)
{
}

void OdGlesJsonServerImpl::setOutput(OdStreamBuf* buf)
{
  m_pOutStream = buf;
}

// set output path name or format(if %d is present in path)
bool OdGlesJsonServerImpl::setOutPathName(const OdString& sPathNameFormat,
...
int OdGlesJsonServerImpl::flushOut() // return next free index
{
  m_pOutStream = NULL;
  return m_indexNextFree;
}

void OdGlesJsonServerImpl::ident(const char* pTag, const char* pText, const OdGLES2JsonServer::JsonType &type)
{
  ...
  if (pTag)
  {
   ...
   if (pText)
   {
     if (type == OdGLES2JsonServer::kArray)
       m_pOutStream->putByte((OdUInt8('[')));
     else if (type == OdGLES2JsonServer::kType)
       m_pOutStream->putByte((OdUInt8('{')));
     else if (type == OdGLES2JsonServer::kString)
       m_pOutStream->putByte((OdUInt8('"')));
    }
  }
