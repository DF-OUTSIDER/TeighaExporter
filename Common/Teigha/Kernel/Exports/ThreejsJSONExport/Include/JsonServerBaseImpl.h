...
///////////////////////////////////////////////////////////////////////////////
// Base implementation of JsonServer interface for GLES2 Json device

#ifndef ODGLES2JSONSERVERBASEIMPL
#define ODGLES2JSONSERVERBASEIMPL

#include "TD_PackPush.h"

#include "JsonServer.h"
#include "Tr/TrVisUniqueId.h"
...
*/
class OdGLES2JsonServerBaseImpl : public OdGLES2JsonServer, protected OdTrVisUniqueID::DefUIDTransformer
{
protected:
  int m_nIdent;
  bool m_bClientDependentSectionOnly;
  OdDbBaseDatabasePE *m_pDbPE;
  OdBool m_bEmptyMetaFile;
  OdBool m_bEnableFaces;
...
public:
  OdGLES2JsonServerBaseImpl(const OdDbBaseDatabase *pDb = NULL)
    : m_nIdent(0)
    , m_bClientDependentSectionOnly(false)
    , m_pDbPE(NULL)
    , m_pszTmpBuf(NULL)
    , m_lenTmpBuf(0)
    , m_bEmptyMetaFile(true)
    , m_bEnableFaces(true)
  {
    setDatabase(pDb);
  }
  ...
  void setEnableFaces(OdBool enable) 
  {
    m_bEnableFaces = enable;
  }

  OdBool isEnableFaces() const
  {
    return m_bEnableFaces;
  }
  ...
  virtual void ident(const char* pTag, const char* pText, const OdGLES2JsonServer::JsonType &)  = 0;

  virtual void IncreaseNesting(const char* pLevelName, const OdGLES2JsonServer::JsonType &JsonType)
  {
    ODFMTBEG((odStrLenA(pLevelName) > 0 ? odStrLenA(pLevelName) + 4 : 1))
    odStrLenA(pLevelName) > 0 ?
    (JsonType == OdGLES2JsonServer::kArray ? "\"%s\":[" : "\"%s\":{") : (JsonType == OdGLES2JsonServer::kArray ? "[" : "{"), pLevelName
      ODFMTEND()
     ident(NULL, m_sTmpBuf.c_str(), JsonType);
     m_nIdent += 2;
  }
  virtual void DecreaseNesting(const char* pLevelName, const OdGLES2JsonServer::JsonType &JsonType)
  {
    m_nIdent -= 2;
    ODFMTBEG(2)
    //"}", pLevelName
    (JsonType == OdGLES2JsonServer::kArray ? "]" : "}")
    ODFMTEND()
    ident(NULL, m_sTmpBuf.c_str(), JsonType);
  }
                      ...
                      OdInt32 enAdv = -1)
  {
    ...
    OdGLES2JsonServer::DropId(pTagName, data, pId, enAdv); // DropUInt64(pTagName, data);
  }

  virtual void DropBoolean(const char* pTagName, bool data)
  {
    //DropUInt32(pTagName, (data) ? 1 : 0);
    m_sTmpBuf = data ? "true" : "false";
    ident(pTagName, m_sTmpBuf.c_str(), OdGLES2JsonServer::kNumber);
  }
  ...
  virtual void DropChars(const char* pTagName, const char *pStr)
  {
    ...
    m_sTmpBuf.replace(">", "&gt;");
    m_sTmpBuf.replace("\n", "&#x0A;");
    ident(pTagName, m_sTmpBuf.c_str(), OdGLES2JsonServer::kString);
  }
  ...
  virtual void DropInts(const char* pTagName, OdUInt32 nData, const OdUInt16* pData)
  {
    ...
    ODFMTEND()
    ident(pTagName, m_sTmpBuf.c_str(), OdGLES2JsonServer::kArray);
  }

  virtual void DropUInts(const char* pTagName, OdUInt32 nData, const OdUInt32* pData)
  {
    ODA_ASSERT_ONCE(nData && pData);
    ODFMTFORBEG(32)
      index ? ",%u" : "%u", pData[index]
      ODFMTEND()		
      ident(pTagName, m_sTmpBuf.c_str(), OdGLES2JsonServer::kArray);
  }
  ...
  virtual void DropDbStub(const char *pTagName, OdDbStub *pDbStub)
  {
    ...
    ident(pTagName, m_sTmpBuf, OdGLES2JsonServer::kType);
  }
...
#endif // ODGLES2JSONSERVERBASEIMPL
