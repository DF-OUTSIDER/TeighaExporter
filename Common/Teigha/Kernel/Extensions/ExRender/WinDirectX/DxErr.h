...
// DirectX 8 & 9 Error Functions
/* Originally written by Filip Navara 
...
static DxError UnknownError = DXERRORLAST(0xffffffff, "Unknown", "n/a");

static DxError *FindError(unsigned long hr)
{
  register DxError *Base = &Errors[0];
  register int Limit;
  register DxError *Ptr;

  for (Limit = sizeof(Errors) / sizeof(DxError); Limit != 0; Limit >>= 1)
  {
    Ptr = Base + (Limit >> 1);
    if (hr == Ptr->Value)
      return Ptr;
...
static const TCHAR *DXGetErrorString(unsigned long hr)
{
  return FindError(hr)->Name;
}

const TCHAR *DXGetErrorDescription(unsigned long hr)
{
