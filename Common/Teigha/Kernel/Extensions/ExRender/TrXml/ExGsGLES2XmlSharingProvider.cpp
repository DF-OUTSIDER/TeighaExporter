...
OdGLES2XmlSharingProvider::OdGLES2XmlSharingProvider()
...
OdGLES2XmlRenditionRedir *OdGLES2XmlSharingProvider::rendition(OdTrVecDevice *pRequestFor)
{
  return static_cast<OdGLES2XmlRenditionRedir*>(OdTrVecResourceSharingProvider::rendition(pRequestFor));
}

// Share metafile with secondary device (should be overrided by client, do nothing by default)
bool OdGLES2XmlSharingProvider::shareMetafile(OdTrVecDevice *pRequestedFor, OdTrVecDevice *pRequestedFrom,
                                              OdTrVisMetafileId metafileId, void * /*pInternalDataPtr*/, void * /*pOriginatorPtr*/)
{
  ...
  if (pRegId) rendition(pRequestedFor)->xmlRendition().xml()->RegisterId(metafileId, pRegId, OdGLES2XmlServer::kMetafileId);
...
bool OdGLES2XmlSharingProvider::shareTexture(OdTrVecDevice *pRequestedFor, OdTrVecDevice *pRequestedFrom,
                                             OdTrVisTextureId textureId, void *pInternalDataPtr, void *pOriginatorPtr)
{
  ...
  if (pRegId) rendition(pRequestedFor)->xmlRendition().xml()->RegisterId(textureId, pRegId, OdGLES2XmlServer::kTextureId);
...
bool OdGLES2XmlSharingProvider::shareMaterial(OdTrVecDevice *pRequestedFor, OdTrVecDevice *pRequestedFrom,
                                              OdTrVisMaterialId materialId, void *pInternalDataPtr, void *pOriginatorPtr)
{
  ...
  if (pRegId) rendition(pRequestedFor)->xmlRendition().xml()->RegisterId(materialId, pRegId, OdGLES2XmlServer::kMaterialId);
