...
class CPDF_ImageRenderer {
 public:
  CPDF_ImageRenderer();
  virtual ~CPDF_ImageRenderer();

  virtual bool Start(CPDF_RenderStatus* pStatus,
  ...
  virtual bool Continue(IFX_PauseIndicator* pPause);
 ...
 protected:
  ...
  virtual bool DrawPatternImage(const CFX_Matrix* pObj2Device);
  bool NotDrawing() const;
  FX_RECT GetDrawRect() const;
  CFX_Matrix GetDrawMatrix(const FX_RECT& rect) const;
  virtual void CalculateDrawImage(CFX_DefaultRenderDevice* bitmap_device1,
