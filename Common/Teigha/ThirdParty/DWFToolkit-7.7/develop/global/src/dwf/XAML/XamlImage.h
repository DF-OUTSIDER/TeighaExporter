...
#if !defined XAML_IMAGE_HEADER
#define XAML_IMAGE_HEADER

///
///\file        XAML/XamlImage.h
///
 ...
 */
class XAMLTK_API WT_XAML_Image : public WT_Image
{
	friend class WT_XAML_Class_Factory;
protected:
    // Constructors, Destructors
    /// Constructs a WT_XAML_Image object.
    WT_XAML_Image ()
       : WT_Image()                
    { }

    /// Constructs a WT_XAML_Image object of the given type (one of \ref WT_Image_Format "enum WT_Image_Format".)
    WT_XAML_Image (int image_type)
       : WT_Image(image_type)
    { }


    /// Constructs a WT_XAML_Image object with the given data.
    WT_XAML_Image(
		...
		: WT_Image(rows,columns,format,identifier,color_map,data_size,data,min_corner,max_corner,copy,dpi)
	{ }

	WT_XAML_Image & operator= (WT_XAML_Image const &)
    {
        WD_Complain ("cannot assign WT_XAML_Image");
        return *this;
    } // prohibited

    /// Destroys a WT_XAML_Image object.
    virtual ~WT_XAML_Image()
    { }
public:
