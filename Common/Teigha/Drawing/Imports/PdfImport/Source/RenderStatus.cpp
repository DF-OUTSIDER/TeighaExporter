...
OdRenderStatus::OdRenderStatus(ByteString& current_layer_name, bool& is_object_visible)
  :CPDF_RenderStatus()
...
void OdRenderStatus::RenderSingleObject(CPDF_PageObject* pObj, const CFX_Matrix* pObj2Device) 
{
  ...
  std::vector<ByteString> layers_cache;
  bool visible = true;
  if (m_Options.GetOCContext() && pObj->m_ContentMark.HasRef()) 
  {
    ...
    for (size_t i = 0; i < pObj->m_ContentMark.CountItems(); ++i) 
    {
      ...
      if (item.GetName() == "OC" && item.GetParamType() == CPDF_ContentMarkItem::PropertiesDict)
      {
        ...
        if (pDict)
        {
          layers_cache.emplace_back(ByteString::FromUnicode(pDict->GetUnicodeTextFor("Name")));
        }
      }
    }
  }
  if (0 != layers_cache.size())
    m_CurrentLayerName = layers_cache.back();
  else
...
void OdRenderStatus::DrawObjWithBackground(CPDF_PageObject* pObj, const CFX_Matrix* pObj2Device) 
{
  ...
  OdRenderStatus status(m_CurrentLayerName, m_IsObjectVisible);
...
bool OdRenderStatus::ProcessTransparency(CPDF_PageObject* pPageObj, const CFX_Matrix* pObj2Device)
{
  ...
  OdRenderStatus bitmap_render(m_CurrentLayerName, m_IsObjectVisible);
...
bool SetDIBitsWithBlend(const RetainPtr<CFX_DIBSource>& pBitmap, int left, int top, int blend_mode, OdDbDeviceDriver* driver, FX_RECT& clip_rect)
{
...
void OdRenderStatus::DrawShading(CPDF_ShadingPattern* pPattern, CFX_Matrix* pMatrix, FX_RECT& clip_rect, int alpha, bool bAlphaMode)
{
  ...
  OdDbDeviceDriver* driver = dynamic_cast<OdDbDeviceDriver*>(m_pDevice->GetDeviceDriver());
  if(nullptr != driver)
