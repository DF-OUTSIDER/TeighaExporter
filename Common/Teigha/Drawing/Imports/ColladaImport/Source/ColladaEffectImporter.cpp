...
namespace TD_COLLADA_IMPORT
{
	...
	bool EffectImporter::import(const COLLADAFW::Effect* pEffect)
	{
		const COLLADAFW::UniqueId& effectUniqueId = pEffect->getUniqueId();
		if ( effectUniqueId.isValid() )
			addUniqueIdFWEffectPair(effectUniqueId, *pEffect);
		return true;
	}
}
