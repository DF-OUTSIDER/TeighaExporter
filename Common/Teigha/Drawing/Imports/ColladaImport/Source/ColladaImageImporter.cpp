...
namespace TD_COLLADA_IMPORT
{
	...
	bool ImageImporter::import(const COLLADAFW::Image* pImage)
	{
		const COLLADAFW::UniqueId& imageUniqueId = pImage->getUniqueId();
		if ( imageUniqueId.isValid() )
			addUniqueIdFWImagePair(imageUniqueId, *pImage);
		return true;
	}
}
