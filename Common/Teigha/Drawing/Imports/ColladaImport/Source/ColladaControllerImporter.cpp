...
namespace TD_COLLADA_IMPORT
{
  ...
  bool ControllerImporter::import(const COLLADAFW::Controller & pController)
  {
    const COLLADAFW::UniqueId& imageUniqueId = pController.getUniqueId();
    if (imageUniqueId.isValid())
      addUniqueIdFWControllerPair(imageUniqueId, pController);
     return true;
  }
}
