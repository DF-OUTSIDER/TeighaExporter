...
ODRX_DEFINE_MEMBERS_EX(OdDbIdwVersion,     // ClassName
                       ...
                       L"AcDbIdwVersion",  // DWG class name
                       L"ACDBIDWVERSION",  // DxfName
                       L"AcSynergyObjDPW", // AppName
                       OdRx::kMTLoading | OdRx::kHistoryAware);

OdDbIdwVersion::OdDbIdwVersion(bool createImpObj)
 : OdDbObject()
 , m_pImpObj(NULL)
{
  if (createImpObj)
    m_pImpObj = new OdDbImpIdwVersion();
}

OdDbIdwVersion::~OdDbIdwVersion()
{
  if (m_pImpObj)
    delete m_pImpObj;
}

OdResult OdDbIdwVersion::dwgInFields(OdDbDwgFiler* pFiler)
{
...
void OdDbIdwVersion::dwgOutFields(OdDbDwgFiler* pFiler) const
{
  OdDbObject::dwgOutFields(pFiler);
  m_pImpObj->dwgOutFields(pFiler, objectId());
}

OdResult OdDbIdwVersion::dxfInFields(OdDbDxfFiler* pFiler)
{
  ODA_ASSERT_ONCE(m_pImpObj);
  if (!m_pImpObj)
    return eMakeMeProxy;
...
void OdDbIdwVersion::dxfOutFields(OdDbDxfFiler* pFiler) const
{
  OdDbObject::dxfOutFields(pFiler);
  m_pImpObj->dxfOutFields(pFiler, objectId());
}

OdUInt32 OdDbIdwVersion::getVersion() const
{
  assertReadEnabled();
  return m_pImpObj->m_vession;
}
